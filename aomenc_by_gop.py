import subprocess, os, sys, vapoursynth, re, traceback, argparse, platform
...
class Worker:
  def __init__(self, queue, args, passes, script, update1, update2):
    self.queue = queue
    self.args = args
    self.passes = passes
    self.script = script
  ...
  def encode(self, segment):
    vspipe_cmd = [
      "vspipe", self.script, "-y", "-", "-s",
      str(segment[0]), "-e",
      str(segment[1])
    ]

    aomenc_cmd = [
      "aomenc", "-", "--ivf", "-o", "segment_{}.ivf".format(segment[2]),
...
def concat(n_segments, output):
  ...
  out = _concat(segments, output, file_limit, cmd_limit)
...
def _concat(files, output, file_limit, cmd_limit, flip=False):
  ...
  if len(remaining) > 0:
    return _concat([tmp_out] + remaining, output, file_limit, cmd_limit,
                   not flip)
  else:
    return tmp_out
...
core.fmtc.bitdepth(resized, bits=8).set_output()"""
  ...
  for i in range(args.workers):
    workers.append(
      Worker(queue, aom_args, args.passes, script_name, progress_bar.update,
             update))
  ...
  vspipe = subprocess.Popen(get_gop,
                            stdout=subprocess.PIPE,
                            creationflags=subprocess.CREATE_NO_WINDOW)
